...
export class HunterChatGPTMCP extends McpAgent {
	...
	async init() {
		...
		this.server.registerResource(
			"discover-widget",
			"ui://widget/discover-widget.html",
			{},
			async () => ({
				contents: [
					{
			...
			"discover",
			{
				...
				inputSchema: {
					query: z
						.string()
						.describe(
							"Your search query. Example: 'software companies in San Francisco with more than 50 employees'",
						),
				},
		...
		this.server.registerTool(
			"save",
			{
				...
				inputSchema: {
					domain: z
						.string()
						.describe("Domain name of the company to save into your Hunter Leads"),
				},
