...
function extractCompanyFromGlobals(globals: GlobalsState): CompanyData {
  const raw =
    (globals.toolOutput?.data as CompanyData | undefined) ||
    (globals.toolOutput as CompanyData | undefined) ||
    {};

  const alreadySaved =
    (raw as unknown as { already_saved?: boolean }).already_saved ??
    (raw as CompanyData).alreadySaved;
...
function App() {
  ...
  const emailCount =
    company.emailsCount?.total ?? company.site?.emailAddresses?.length ?? 0;
    ...
    <div className="w-full max-w-sm rounded-2xl bg-[var(--color-surface)] border border-[0.5px] border-[var(--color-border-primary-outline)] shadow-[var(--shadow-300)]">
      ...
      <div className="p-4 space-y-3 pt-2">
        ...
        <div className="flex justify-between items-baseline">
          <h1 className="text-xl font-medium text">
        ...
        <div className="flex gap-3 py-2">
          ...
          <Button
            ...
            size="lg"
            variant="soft"
            onClick={() => handleViewInHunterClick(company.domain)}
          >
            View in Hunter
          </Button>
        </div>
