...
export function registerLeadsListTools(server: McpServer, apiKey: string, baseUrl: string) {
  server.registerTool(
    "List-Leads-Lists",
    {
      description: "List all leads lists in your Hunter account",
      inputSchema: {
  ...
  server.registerTool(
    "Get-Leads-List",
    {
      description: "Get a single leads list by ID",
      inputSchema: {
  ...
  server.registerTool(
    ...
    async ({ name }) => {
      return callHunterApi({ path: "/leads_lists", apiKey, baseUrl, method: "POST", params: { name } })
    },
  )
  ...
  server.registerTool(
    "Update-Leads-List",
    {
      description: "Rename an existing leads list",
      inputSchema: {
  ...
  server.registerTool(
    ...
    {
      description:
        "Merge one leads list into another. All leads from the source list are moved to the destination list, and the source list is deleted.",
      inputSchema: {
