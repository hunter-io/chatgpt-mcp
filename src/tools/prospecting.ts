  ...
  server.registerTool(
    ...
    async ({ query: _query }) => {
      ...
      const directives = [
        ...
        "Before issuing the first Domain-Search call with `pending_companies`, summarise the expected credit cost to the user and wait for approval: each picked company costs roughly 1 search credit (Domain-Search) plus up to 1 verification credit (Email-Verifier per saved valid email). This is the single confirmation point for the whole loop. After the user approves, set `confirmed_credit_use: true` on the Domain-Search call to authorize the batch; the server-side guard returns an ask_user without that flag, and subsequent chained calls carry the flag forward automatically. Do not pause between companies after approval.",
