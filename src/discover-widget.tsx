...
import { Plus, Check, BuildingWorkspace } from "@openai/apps-sdk-ui/components/Icon"
...
function CompanyLogo({ logo }: { logo?: string }) {
  ...
  return (
    <div className="w-full h-full bg-[var(--color-surface-primary)]">
      <img src={logo} alt="" className="w-full h-full object-cover" onError={() => setImageError(true)} />
    </div>
  )
}
...
function CompanyListItem({ company }: { company: DiscoverCompany }) {
  const companyName = company.organization || company.domain || "Unknown Company"
  const emailCount = company.emails_count?.total ?? 0
  const emailText =
    emailCount > 0 ? `${emailCount.toLocaleString()} email${emailCount === 1 ? " address" : " addresses"}` : null

  // Construct logo URL from domain (logo is always constructed from domain)
  const logoUrl = company.domain ? `https://company-logo.hunter.io/${company.domain}` : undefined
    ...
    <li className="flex items-center gap-3 px-4 py-3 border-b border-[var(--color-border-primary-soft-alt)] hover:bg-[var(--color-surface-secondary)] transition-colors">
      ...
      <div className="flex-1 min-w-0">
        <span className="flex items-center">
          ...
          {company.hiring && <span className="text-secondary">&nbsp;· Hiring</span>}
        </span>
        {(emailText || company.industry || company.location) && (
          <span className="text-sm text-secondary flex items-center gap-1 block">
            {emailText && <span className="whitespace-nowrap">{emailText}</span>}
            ...
            {company.location && <span className="whitespace-nowrap">{company.location}</span>}
          </span>
        )}
      </div>
  ...
  return (
    <div className="relative w-[44px] h-[44px]" aria-hidden="true">
      {[
        ...
        const bgClass = isLogo ? "bg-[var(--color-text-primary-solid)]" : "bg-[var(--color-background-primary-soft)]"
    ...
    <div className="w-full rounded-2xl border border-[0.5px] border-[var(--color-border-primary-outline)] shadow-[var(--shadow-300)] p-4">
      ...
      <div className="mb-2">
        <h2 className="text-lg font-medium text">
          {totalResults.toLocaleString()} {totalResults === 1 ? "company" : "companies"} match your filters
        </h2>
      </div>
      ...
      {remainingCount > 0 && (
        <footer className="flex items-center justify-between pt-2">
          <div className="flex items-center gap-3">
            ...
            <span className="text-secondary">+ {remainingCount.toLocaleString()} more</span>
          </div>
          <Button color="primary" size="sm" variant="outline" onClick={handleViewInHunterClick}>
            Open in Hunter
          </Button>
        </footer>
      )}
